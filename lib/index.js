 ...
 */
import { ghAvailable, ghToken, installPlugin, installedOverview, loadSettings, MarketError, profileDirOf, saveSettings, searchMarket, uploadPlugin } from "./market.js";
...
export function apply(ctx, rawConfig) {
  ...
  const initialSettings = loadSettings(dshHome);
  const config = {
    ...initialSettings,
  ...
  const readSettings = () => {
    const stored = loadSettings(dshHome);
    // 部署 patch 中的静态配置优先于存储，存储可被 UI 覆盖
    return { ...stored, ...(config.proxyUrl !== undefined ? { proxyUrl: config.proxyUrl } : {}) };
  };
  const currentProfile = () => config.profile || "web";
